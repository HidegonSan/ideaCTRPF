...
{
  void DrawLine(const Screen &scr, int srcX, int srcY, int dstX, int dstY,
                const Color &color)
  {
  ...
  void DrawPlus(const Screen &scr, const std::string &str, u32 posX, u32 posY,
                u32 borderWidth, u32 padding, const Color &foreground,
                const Color &background, const Color &border, int fontAlign)
  {
    int bgWidth = OSD::GetTextWidth(false, str);
    int height = 10 + padding * 2;

    scr.DrawRect(posX, posY, bgWidth + (borderWidth * 2) + (padding * 2),
                 borderWidth, border);
    scr.DrawRect(posX + borderWidth + padding + bgWidth + padding,
                 posY + borderWidth, borderWidth, height, border);
    scr.DrawRect(posX, posY + borderWidth + height,
                 bgWidth + (borderWidth * 2) + (padding)*2, borderWidth,
                 border);
    scr.DrawRect(posX, posY + borderWidth, borderWidth, height, border);

    scr.DrawRect(posX + borderWidth, posY + borderWidth, bgWidth + padding * 2,
                 10 + padding * 2, background);
  ...
  void DrawSysfontPlus(const Screen &scr, const std::string &str, u32 posX, u32 posY,
                       u32 borderWidth, u32 padding, const Color &foreground,
                       const Color &background, const Color &border,
                       bool fillBackground, bool rightAligned, int fontAlign)
  {
    int bgWidth = OSD::GetTextWidth(true, str);
    int height = 16 + padding * 2;
    if (rightAligned)
    {
      scr.DrawRect(posX - bgWidth - (borderWidth * 2) - (padding), posY,
                   bgWidth + (borderWidth * 2) + (padding * 2), borderWidth,
                   border);
      scr.DrawRect(posX - borderWidth, posY + borderWidth, borderWidth,
                   height, border);
      scr.DrawRect(posX - bgWidth - (borderWidth * 2) - (padding),
                   posY + borderWidth + height,
                   bgWidth + (borderWidth * 2) + (padding)*2, borderWidth,
                   border);
      scr.DrawRect(posX - bgWidth - (borderWidth * 2) - (padding),
                   posY + borderWidth, borderWidth, height, border);
      if (fillBackground)
      {
        scr.DrawRect(posX + borderWidth - bgWidth, posY + borderWidth,
                     bgWidth + padding * 2, 16 + padding * 2, background);
      }
      scr.DrawSysfont(str, posX + borderWidth + padding - bgWidth,
                      posY + borderWidth + padding, foreground);
    }
    else
    {
      scr.DrawRect(posX, posY, bgWidth + (borderWidth * 2) + (padding * 2),
      ...
      if (fillBackground)
      {
        scr.DrawRect(posX + borderWidth, posY + borderWidth,
                     bgWidth + padding * 2, 16 + padding * 2, background);
      }
  ...
  void DrawCircle(const Screen &scr, u32 x, u32 y, u32 radiusStart, u32 radiusEnd,
                  int start, int end, const Color &color, int origin)
  {
    ...
    for (int r = miniRadius; r < radiusEnd; r++)
    {
      ...
      {
        scr.DrawPixel(x + cos(DegreeToRadian(angle)) * r,
                      y + sin(DegreeToRadian(angle)) * r, color);
      }
    }
  }

  void DrawRectPlus(const Screen &scr, u32 posX, u32 posY, u32 width, u32 height,
                    const Color &color, bool filled, int origin)
  {
    if (origin == 0)
    {
    }
  ...
  {
    Process::WriteString((u32)U16_ChrArray,
                         _flick ? KatakanaMode ? FlickKatakana : FlickHiragana : KatakanaMode ? Katakana
                                                                                              : Hiragana,
                         StringFormat::Utf16);
  }
  ...
  bool JPKeyboard::DrawKeyboard(const Screen &scr, std::string &out)
  {
    ...
    if (_flick)
      for (u8 x = 0; x < 3; x++)
        for (u8 y = 0; y < 4; y++)
        {
          ...
          u16 sy = 70 + y * 30;
          key.clear();
          Process::ReadString((u32)&U16_ChrArray[(y * 3 + x) * 5], key, 2,
                              StringFormat::Utf16);
      ...
      for (u8 x = 0; x < 10; x++)
        for (u8 y = 0; y < 5; y++)
        {
          ...
          // 文字
          key.clear();
          Process::ReadString((u32)&U16_ChrArray[y * 10 + x], key, 2,
                              StringFormat::Utf16);
          scr.DrawSysfont(key, sx + 5, sy + 3, Color::White);
        }
    ...
    if (!InputChrs.empty())
    {
      ...
      Process::ReadString((u32)str, InputStr, InputChrs.size() * 2,
                          StringFormat::Utf16);
      u8 i = 0;
      u8 j = 0;
      ...
      scr.DrawRect(58 + width - selectedIndex * 13, 37, selectedIndex * 13,
                   17, Color::Blue);
    ...
    if (_canConvert)
    {
      ...
      if (Controller::IsKeyPressed(Touchpad) && TouchRect(32, 32, 24, 22))
      {
        ...
        std::vector<u16> lastN(InputChrs.end() - selectedIndex - 1,
                               InputChrs.end());
        if (lastN[0] > 0x1000)
        {
          selectedIndex++;
      ...
      if (Controller::IsKeyPressed(Key::Y) && selectedIndex != 0)
      {
        ...
        if (!kanji.empty())
        {
          ...
          if (InputChrs.size() < _maxLength)
          {
            ...
            Process::WriteString((u32)buff_utf16, kanji,
                                 StringFormat::Utf16);
            i = 0;
            selectedIndex = 0;
            while (1)
      ...
      if (KatakanaMode)
      {
        ...
        for (int i = 0; i < input.length(); i++)
        {
          if (InputChrs.size() > _maxLength)
            break;
          u16 buff;
          Process::WriteString((u32)&buff, input.substr(i, 1),
                               StringFormat::Utf16);
      ...
      if (_flick)
      {
        if (pos.x >= 120 && pos.y >= 70 && pos.x <= 210 && pos.y <= 190)
        {
          ...
          while (Touch::IsDown())
          {
            ...
            {
              i = 1;
              scr.DrawRect(120 + wx * 30 - 20, 70 + wy * 30, 20, 20,
                           Color::White);
              key.clear();
              Process::ReadString(
                  (u32)&U16_ChrArray[(wy * 3 + wx) * 5 + 1], key, 2,
                  StringFormat::Utf16);
            ...
            {
              i = 2;
              scr.DrawRect(120 + wx * 30, 70 + wy * 30 - 20, 20, 20,
                           Color::White);
              key.clear();
              Process::ReadString(
                  (u32)&U16_ChrArray[(wy * 3 + wx) * 5 + 2], key, 2,
                  StringFormat::Utf16);
            ...
            else if (posX < -10)
            {
              ...
              key.clear();
              Process::ReadString(
                  (u32)&U16_ChrArray[(wy * 3 + wx) * 5 + 3], key, 2,
                  StringFormat::Utf16);
            ...
            else if (posY < -10)
            {
              ...
              key.clear();
              Process::ReadString(
                  (u32)&U16_ChrArray[(wy * 3 + wx) * 5 + 4], key, 2,
                  StringFormat::Utf16);
            ...
            {
              i = 0;
              scr.DrawRect(120 + wx * 30, 70 + wy * 30, 20, 20,
                           Color::White);
              key.clear();
              Process::ReadString(
                  (u32)&U16_ChrArray[(wy * 3 + wx) * 5], key, 2,
                  StringFormat::Utf16);
  ...
  std::vector<u16>
  Convert::strToSjis(std::string str)
  {
  ...
  std::string
  Convert::hiraganaToKatakana(std::string hiragana)
  {
  ...
  std::string
  Convert::katakanaToHiragana(std::string katakana)
  {
  ...
  std::vector<Color>
  drawVerticalGradation(u16 x, u16 w, u16 y, u16 h, Color start, Color end,
                        Screen scr)
  {
  ...
  bool DrawColorPicker(Screen scr, Color &out)
  {
    ...
    buff_vector = drawVerticalGradation(260, 15, 24, 32, Color(255, 0, 0),
                                        Color(255, 255, 0), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    buff_vector = drawVerticalGradation(260, 15, 56, 32, Color(255, 255, 0),
                                        Color(0, 255, 0), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    buff_vector = drawVerticalGradation(260, 15, 88, 32, Color(0, 255, 0),
                                        Color(0, 255, 255), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    buff_vector = drawVerticalGradation(260, 15, 120, 32, Color(0, 255, 255),
                                        Color(0, 0, 255), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    buff_vector = drawVerticalGradation(260, 15, 152, 32, Color(0, 0, 255),
                                        Color(255, 0, 255), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    buff_vector = drawVerticalGradation(260, 15, 184, 32, Color(255, 0, 255),
                                        Color(255, 0, 0), scr);
    barColorVector.insert(barColorVector.end(), buff_vector.begin(),
                          buff_vector.end());
    ...
    DrawSysfontPlus(scr, "H", 150, 30, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(
        scr,
        Utils::Format("%d", (int)((double)(barLength - 24) / 192.0 * 360.0)),
        230, 30, 0, 0, Color::Black, Color::Black, Color::Red, false, true, 8);
    DrawRectPlus(scr, 160, 50, 70, 19, Color::White, true, 0);
    DrawSysfontPlus(scr, "S", 150, 50, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(scr, Utils::Format("%d", pickerPos.x - 26), 230, 50, 0, 0,
                    Color::Black, Color::Black, Color::Red, false, true, 8);
    DrawRectPlus(scr, 160, 70, 70, 19, Color::White, true, 0);
    DrawSysfontPlus(scr, "V", 150, 70, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(scr, Utils::Format("%d", pickerPos.y - 26), 230, 70, 0, 0,
                    Color::Black, Color::Black, Color::Red, false, true, 8);
    DrawRectPlus(scr, 160, 120, 70, 19, Color::White, true, 0);
    DrawSysfontPlus(scr, "R", 150, 120, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(scr, Utils::Format("%d", barColor.r), 230, 120, 0, 0,
                    Color::Black, Color::Black, Color::Red, false, true, 8);
    DrawRectPlus(scr, 160, 140, 70, 19, Color::White, true, 0);
    DrawSysfontPlus(scr, "G", 150, 140, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(scr, Utils::Format("%d", barColor.g), 230, 140, 0, 0,
                    Color::Black, Color::Black, Color::Red, false, true, 8);
    DrawRectPlus(scr, 160, 160, 70, 19, Color::White, true, 0);
    DrawSysfontPlus(scr, "B", 150, 160, 0, 0, Color::White, Color::Black,
                    Color::Red, true, 8, 0);
    DrawSysfontPlus(scr, Utils::Format("%d", barColor.b), 230, 160, 0, 0,
                    Color::Black, Color::Black, Color::Red, false, true, 8);
    ...
    scr.DrawSysfont(Utils::Format("#%02X%02X%02X", selectedColor.r,
